...
const Navbar = ({ title }) => {
  const { isOpen, setIsopen,setHumber} =
    ...
    <nav className=" navbar transition-all  bg-neutral text-neutral-content mb-16  ">
      <div className=" mx-12 w-full justify-between   flex  max-lg:mx-0  max-sm:!px-2  js-s">
        <div
          ...
          className={`closemm mr-2 btn-sm  z-[1001] !text-base ${
            isOpen ? "open" : ""
          }`}
          id="nav-icon4"
        >
          <span
            className={`${theme ? " bg-[#3abff8]" : "bg-slate-100"}`}
          ></span>
          <span
            className={` ${theme ? " bg-[#3abff8]" : "bg-slate-100"}`}
          ></span>
        ...
        <div className={`felx-1 px-2 flex flex-row dark-response `}>
          <div className="flex items-center gap-3 ss ">
            <NavLink
              to="/"
              activeClassName="cc"
              className={`   btn btn-ghost btn-accent  btn-sm rounded-btn text-[0.90rem]`}
            >
              Home
            </NavLink>
            <NavLink
              to="/about"
              activeClassName="cc"
              className={` btn btn-ghost  btn-sm rounded-btn text-[0.90rem] `}
            >
              About
            </NavLink>
